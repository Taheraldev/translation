...
# تمت إزالة الاستيراد من googletrans
...
# إنشاء فئة تغليف لمحاكاة واجهة googletrans
class TranslatorWrapper:
    def translate(self, text, src='en', dest='ar'):
        try:
            # استخدام deep_translator للترجمة
            translation = DeepGoogleTranslator(source=src, target=dest).translate(text)
            return type('Translated', (), {'text': translation})()
        except Exception as e:
            logger.error(f"خطأ أثناء الترجمة: {e}")
            return type('Translated', (), {'text': text})()

# إنشاء مثيل للمترجم باستخدام فئة التغليف
